...
import com.jms.dboard.manage.jumin.dao.JuminBoardDao;
...
@Service("juminBoardService")
public class JuminBoardServiceImpl implements JuminBoardService{

	@Autowired
	JuminBoardDao juminBoardDao;
...
//			System.out.println(param.getSearchContentType().get(0));
			if(param.getUserId() != null) {
				list = juminBoardDao.getJuminBoardList(param);
				totalCount = juminBoardDao.getTotalJuminBoardList(param);
			}
			...
			e.printStackTrace();
			resultCode = 500;
			resultMsg = "주민소식 조회 중 오류가 발생했습니다.";
		}
	...
	public JSONObject saveJuminBoardInfo(PromoBoardVO param) {
		...
		try {
			...
			if(userInfo != null) {
				...
				int contentId = 0;
				resultCount = juminBoardDao.saveJuminBoardInfo(param);
				contentId =param.getContentId();
				if(param.getFile() != null) {				
					if(param.getContentType().equals("001")) {				
						fileType = "IMG";
					...
					param.setContentPath("/resources/jumin/"+currentDate);
					param.setContentName(newFileName);
					param.setOriginFile(param.getFile().getOriginalFilename());
					/* 파일 저장 */
					CommonCodeVO codeVo = new CommonCodeVO();
					codeVo.setCodeName("JUMIN_PATH"); 
					...
					int layoutType = 1;
					System.out.println("param.getContentType() :"+param.getContentType());
					if(param.getContentType().equals("001")) {
...
//									scaledFile.delete();
					...
					param.setLayoutType(layoutType);
					resultJson.put("contentPath","/resources/jumin/"+currentDate+"/"+newFileName);
				} 
				/** 게시위치 정보가 있을 경우 저장 */
	//			System.out.println("getLocation :"+param.getLocation().size());
				if(param.getClient() != null && param.getClient().size() > 0) {
					...
					locationInfo.put("boardType", "007");
					contentManageDao.saveContClientsPromo(locationInfo);
				}
				juminBoardDao.updateJuminBoardInfo(param);
				...
				if(param.getUrgentYn() != null && param.getUrgentYn().equals("Y")) {
					...
					if(currentDate >= Long.parseLong(param.getPostingStart()) && currentDate <= Long.parseLong(param.getPostingEnd())) {
						...
						List<Integer> clientList = juminBoardDao.getClientsByContent(paramData);
						paramData.clear();
				 ...
				 * 사용자 작업 로그
				 * */
				logManageService.saveLogInfo("INFO","I_MNG_JM_0001","주민소식 등록 ["+contentId+"]");
			} else {
			...
			try {
				resultCount = juminBoardDao.deleteJuminBoardInfo(param);
			} catch (Exception e1) {
				// TODO Auto-generated catch block
				e1.printStackTrace();
			}
			...
			e.printStackTrace();
			resultCode = 500;
			resultMsg = "주민소식 저장 중  오류가 발생했습니다.";
			try {
				resultCount = juminBoardDao.deleteJuminBoardInfo(param);
			} catch (Exception e1) {
				// TODO Auto-generated catch block
	...
	public JSONObject updateJuminBoardInfo(PromoBoardVO param) {
		...
		try {
			...
			if(userInfo != null) {
				...
				codeVo.setCodeName("JUMIN_PATH"); 
				codeVo = commonCodeDao.getCommonCodeInfoByName(codeVo);
				PromoBoardResultVO contentInfo = juminBoardDao.getJuminBoardInfo(param);
				if(param.getFile() != null) {
					if(param.getContentType().equals("001")) {
						fileType = "IMG";
...
//					System.out.println("destinationDir :"+destinationDir);
					...
					if(param.getContentType().equals("001")) {
						...
						if(param.getVideoScale().equals("Y")) {
							...
							if(videoConfig != null) {
								...
								if(layoutType == 1) {	
								/*	File oldFile = new File(destinationDir+File.separator+newFileName);
									oldFile.delete();
									System.out.println("세로 영상 Scaling 시작");
									String tmpFilePath = ffmpegUtils.convertVideo(newCreateFile,CommonUtils.padLeftZeros(contentId+"",10)+".mp4", scaleSize);
									File scaledFile = new File(tmpFilePath);
									newCreateFile.delete();
								*/	
	...
	//				param.getFile().transferTo(newCreateFile);
				...
				resultCount = juminBoardDao.updateJuminBoardInfo(param);
				 ...
				 * 사용자 작업 로그
				 * */
				logManageService.saveLogInfo("INFO","I_MNG_JM_0002","주민소식 수정 ["+contentId+"]");
			} else {
				resultJson.put("result", 440);
				resultJson.put("resultMsg", "세션이 만료되어 로그아웃 되었습니다. 다시 로그인 해주세요.");
			}
			...
			e.printStackTrace();
			resultCode = 500;
			resultMsg = "주민소식 수정 중  오류가 발생했습니다.";
		}
		resultJson.put("result", resultCode);
		resultJson.put("resultMsg", resultMsg);
		return resultJson;
	...
	@Override
	public Map<String,Object> deleteJuminBoardInfo(List<Integer> params) {
		// TODO Auto-generated method stub
		Map<String,Object> resultMap = new HashMap<String,Object>();
		int resultCode = 200;
		String resultMsg = "정상적으로 삭제 되었습니다.";
		try {
			int resultCount = juminBoardDao.deleteJuminBoardInfo(params);
			/*
			 * 사용자 작업 로그
			 * */
			logManageService.saveLogInfo("INFO","I_MNG_JM_0003","주민소식 삭제 "+params);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
			resultCode = 500;
			resultMsg = "주민소식 삭제 중  오류가 발생했습니다.";
		}
	...
	@Override
	public Map<String, Object> getJuminBoardInfo(int contentId) {
		// TODO Auto-generated method stub
		Map<String,Object> resultMap = new HashMap<String,Object>();
		int resultCode = 200;
		String resultMsg = "";
		...
		try {
			...
			resultData = juminBoardDao.getJuminBoardInfo(param);
	...
	public int deleteJuminBoardInfo(PromoBoardVO param) {
		// TODO Auto-generated method stub
				...
				try {
					 ...
					 * 사용자 작업 로그
					 * */
					logManageService.saveLogInfo("INFO","I_MNG_JM_0004","주민소식 삭제 ["+param.getContentId()+"]");
				} catch (Exception e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
