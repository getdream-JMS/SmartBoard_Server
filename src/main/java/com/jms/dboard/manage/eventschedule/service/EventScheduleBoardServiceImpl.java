package com.jms.dboard.manage.eventschedule.service;
...
import com.jms.dboard.manage.eventschedule.dao.EventScheduleBoardDao;
...
@Service("eventScheduleBoardService")
public class EventScheduleBoardServiceImpl implements EventScheduleBoardService{

	@Autowired
	EventScheduleBoardDao eventScheduleBoardDao;
...
//			System.out.println(param.getSearchContentType().get(0));
			if(param.getUserId() != null) {
				list = eventScheduleBoardDao.getPromoBoardList(param);
				totalCount = eventScheduleBoardDao.getTotalPromoBoardList(param);
			}
	...
	public JSONObject savePromoBoardInfo(PromoBoardVO param) {
		...
		try {
			...
			if(userInfo != null) {
				...
				int contentId = 0;
				resultCount = eventScheduleBoardDao.savePromoBoardInfo(param);
...
//									scaledFile.delete();
	...
	//			System.out.println("getLocation :"+param.getLocation().size());
				...
				eventScheduleBoardDao.updatePromoBoardInfo(param);
				...
				if(param.getUrgentYn() != null && param.getUrgentYn().equals("Y")) {
					...
					if(currentDate >= Long.parseLong(param.getPostingStart()) && currentDate <= Long.parseLong(param.getPostingEnd())) {
						...
						List<Integer> clientList = eventScheduleBoardDao.getClientsByContent(paramData);
						paramData.clear();
			...
			try {
				resultCount = eventScheduleBoardDao.deletePromoBoardInfo(param);
			} catch (Exception e1) {
				// TODO Auto-generated catch block
				e1.printStackTrace();
			}
	...
	public JSONObject updatePromoBoardInfo(PromoBoardVO param) {
		...
		try {
			...
			if(userInfo != null) {
				...
				codeVo.setCodeName("PRO_PATH"); 
				codeVo = commonCodeDao.getCommonCodeInfoByName(codeVo);
				PromoBoardResultVO contentInfo = eventScheduleBoardDao.getPromoBoardInfo(param);
				if(param.getFile() != null) {
					if(param.getContentType().equals("001")) {
						fileType = "IMG";
					...
					param.setOriginFile(param.getFile().getOriginalFilename());
					param.setContentName(newFileName);
					File newCreateFile = CommonUtils.uploadFile(param.getFile(),destinationDir,uploadFileName,CommonUtils.padLeftZeros(contentId+"",10));
					int layoutType = 1;
	//				param.setLayoutType(layoutType);
					if(param.getContentType().equals("001")) {
						BufferedImage bi = null;
						...
						if(param.getVideoScale().equals("Y")) {
							...
							if(videoConfig != null) {
								...
								if(layoutType == 1) {	
									...
									File scaledFile = new File(tmpFilePath);
									System.out.println("newCreateFile :"+newCreateFile);	
	...
	//				param.getFile().transferTo(newCreateFile);
				...
				resultCount = eventScheduleBoardDao.updatePromoBoardInfo(param);
	...
	public Map<String,Object> deletePromoBoardInfo(List<Integer> params) {
		...
		try {
			int resultCount = eventScheduleBoardDao.deletePromoBoardInfo(params);
			/*
			 * 사용자 작업 로그
			 * */
	...
	public Map<String, Object> getPromoBoardInfo(int contentId) {
		...
		try {
			...
			resultData = eventScheduleBoardDao.getPromoBoardInfo(param);
